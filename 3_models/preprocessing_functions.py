...
def undersample_data(df, target, random_state=42, oversampling = 0):
    # Finding the numerosity of each class
    df_class_0 = df[df[target] == 'No']
    df_class_1 = df[df[target] == 'Yes']

    #Undersampling
    num_instances_minority_class = len(df_class_1) + oversampling
...
# ------------preprocess_features_retrieve_df ------------------#

#################################################################


def preprocess_features_retrieve_df(df, target, categorical_columns, numerical_columns, one_hot_encoding=True,
                        numerical_tranformer = 'min_max', test_size=0.07, valid_size=0.08):
    ...
    return (Train_df,Train_df_encoded, Train_labels_encoded), (Valid_df, Valid_df_encoded, 
                                                               Valid_labels_encoded), (Test_df, Test_df_encoded, Test_labels_encoded)
